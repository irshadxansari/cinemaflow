...
export function resetPasswordTemplate(resetPasswordLink: string, userName?: string) {
  return `
  <html>
    <body style="font-family: Arial, sans-serif; background-color: #f5f5f5; margin:0; padding:0;">
      <div style="max-width: 500px; margin: 40px auto; background: #fff; padding: 30px; border-radius: 8px; text-align: center; box-shadow: 0 2px 6px rgba(0,0,0,0.1);">
        <h2 style="color: #333;">Hi ${userName || 'there'},</h2>
        <p style="color: #555;">Click the button below to reset your password. The link is valid for 15 minutes.</p>
        <a href="${resetPasswordLink}" style="display: inline-block; padding: 12px 25px; margin: 20px 0; background-color: #2563eb; color: #fff; text-decoration: none; border-radius: 6px; font-weight: bold;">Reset Password</a>
...
export const verifyEmailTemplate = (
  ...
  subject: "Confirm your FutsalX account",
  html: `
    <html><head><style>
      ...
      .header { background-color: #2563EB; font-weight:bold; font-size: 24px; color: #ffffff; padding: 20px; text-align: center; border-top-left-radius: 8px; border-top-right-radius: 8px; }
      ...
      <div class="container">
        <div class="header">FutsalX</div>
        <div class="content">
          <h1>Confirm Your Email Address</h1>
          <p>Thank you for signing up! Please enter the following verification code when prompted.</p>
